...
//	    cap = (move >> 15) & 0x7;   //captured piece
	...
	String GetConsoleFEN(){
	    //Returns a string of characters that can be printed
	    //to the console displaying the board.
	    ...
	    for(i = num_of_sq_to_display - 8; i >=0; i-=8){
	        ...
	        int k = i + 8;  //set upper bound on next for-loop
	        //		cout << i/8 + 1 << ' '; //Print the rank number
//	        int rankno = i/8 + 1;  1/9/2010 never read locally
	        //		cout << rankno << " | "; //Print the rank number
	        sb.append("  | ");
	        for(j = i; j < k; ++j, mask <<= 1){
	            //If there's a piece at that square
	            //Print PIECE[c][p] otherwise print "-"
	            ...
	            for (int c = 0; c < Color.MAXCOLOR; c++){
	                for (int p = 0; p < Pieces.MAXPIECE; p++){ 
	                    if (Util.bool(mask & pieces[c][p])){
	                        //cout << PIECE_STR[c][p] << ' ';  
	                        //s.append((char*)(PIECE_STR[c][p])); s.append(" ");
	                        nopiece = false;
	                    }
	                }//end for p
	            }//end for c
	            if (nopiece){
	                //cout << ". "; //print "." for white square
	                sb.append(". ");
	            }
	        }
	        //cout << "|\n";
