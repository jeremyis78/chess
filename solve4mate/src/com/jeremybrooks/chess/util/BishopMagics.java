...
public class BishopMagics {
	private static final Logger log = Logger.getLogger(BishopMagics.class);
	private static final Random random = new Random();
	private static Magic[] magics          = new MagicEntry[64];
	private static long[][] movesDatabase = new long[64][512];
	
	static long occupancyMasks[]       = new long[64];
	static long occupancyVariation[][] = new long[64][512];
	static long occupancyAttackSet[][] = new long[64][1428];
	...
	private static void populateOccupancyVariationsAndAttackSets() {
		...
		for (bitRef=0; bitRef<=63; bitRef++)
		{
			...
			for (variationIndex=0; variationIndex<variationCount; variationIndex++)
			{
				...
				for (j=bitRef+9; j%8!=7 && j%8!=0 && j<=55 && (occupancyVariation[bitRef][variationIndex] & (1L << j)) == 0; j+=9);
				if (j>=0 && j<=63) occupancyAttackSet[bitRef][variationIndex] |= (1L << j);
				for (j=bitRef-9; j%8!=7 && j%8!=0 && j>=8 && (occupancyVariation[bitRef][variationIndex] & (1L << j)) == 0; j-=9);
				if (j>=0 && j<=63) occupancyAttackSet[bitRef][variationIndex] |= (1L << j);
				for (j=bitRef+7; j%8!=7 && j%8!=0 && j<=55 && (occupancyVariation[bitRef][variationIndex] & (1L << j)) == 0; j+=7);
				if (j>=0 && j<=63) occupancyAttackSet[bitRef][variationIndex] |= (1L << j);
				for (j=bitRef-7; j%8!=7 && j%8!=0 && j>=8 && (occupancyVariation[bitRef][variationIndex] & (1L << j)) == 0; j-=7);
				if (j>=0 && j<=63) occupancyAttackSet[bitRef][variationIndex] |= (1L << j);
...
//                System.out.println(LongDisplayer.paste(Square.named(bitRef) + " occupied-"+variationIndex, occupiedVariation, "\tAttackSet", attackSet));
    ...
    {
		long[][] magicMoves = new long[64][512];
