...
public class Evaluator {
    ...
    public int evaluate(GameState g, int side, int depth, int[] currentMove, boolean isSearchDebug, boolean isEval){
        ...
        if(g.inCheck()) 
        {
			//TODO: Perhaps this is here because the addition of static exchange evaluation (SEE)
			// was added but not yet incorporated into this main eval function.  Maybe I was still
			// wanting to make sure we didn't evaluate a position
			// until there was some sort of quiescence--not in check, not in the middle of
			// an exchange of pieces.  Somehow we need to incorporate using the SEE in this
			// main eval function.
			System.out.println("we're in check (and throws exception here in the evaluator; " +
					"This feels odd and I think we shouldn't assert/throw):\n" + UciDriver.toDiagram(g));
